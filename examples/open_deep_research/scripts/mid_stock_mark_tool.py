...
class MidStockMarkTool(Tool):
    ...
    description = (
        ...
        and allows highlighting specific dates, adding support lines (in blue) and resistance lines (in red) on the charts.
    ...
    inputs = {
        ...
        "highlight_dates": {
            "type": "string",
            "description": "Optional comma-separated list of dates to highlight on the chart (format: YYYY-MM-DD)",
            "nullable": True
        },
        "support_level": {
            "type": "string",
        ...
        "show_sma5": {
            "type": "boolean",
            "description": "Whether to calculate and display the 5-day Simple Moving Average (default: False)",
            "nullable": True
        },
        "show_sma20": {
            "type": "boolean",
            "description": "Whether to calculate and display the 20-day Simple Moving Average (default: False)",
            "nullable": True
        },
        "show_sma60": {
            "type": "boolean",
            "description": "Whether to calculate and display the 60-day Simple Moving Average (default: False)",
            "nullable": True
        },
        "show_rsi": {
            "type": "boolean",
    ...
    def forward(self, tickers: str, highlight_dates: str = None, 
        ...
        highlight_datetimes = []
        if highlight_dates:
            for date_str in highlight_dates.split(','):
                date_str = date_str.strip()
                try:
                    date_time = pd.to_datetime(date_str)
                    highlight_datetimes.append(date_time)
                    print(f"Will highlight candle for date: {date_time.strftime('%Y-%m-%d')}")
                except Exception as e:
                    print(f"Invalid date format for date: {date_str}. Error: {str(e)}")
                    print("Using format YYYY-MM-DD. Skipping this date.")
        ...
        for ticker in valid_tickers:
            try:
                ...
                # 하이라이트할 날짜들이 있는지 확인하고, 해당 날짜들의 캔들에 마커 표시
                highlight_indices = []
                if highlight_datetimes:
                    ...
                    for highlight_datetime in highlight_datetimes:
                        ...
                        # 정확히 일치하는 날짜 찾기
                        matching_indices = [i for i, date in enumerate(df_dates) if date == target_date]
                        
                        if matching_indices:
                            # 해당 날짜의 데이터가 여러 개 있을 경우(시간별 데이터), 첫 번째 항목만 사용
                            highlight_index = matching_indices[0]
                            highlight_indices.append(highlight_index)
                            print(f"Found matching date at index {highlight_index}: {df.index[highlight_index]}")
                        else:
                            print(f"Date {target_date} not found in the data. Available dates range: {min(df_dates)} to {max(df_dates)}")
                ...
                try:
                    ...
                    # 하이라이트된 날짜 정보 추가
                    highlighted_dates_info = []
                    for idx in highlight_indices:
                        date_info = {
                            "date": df.index[idx].strftime("%Y-%m-%d"),
                            "time": df.index[idx].strftime("%H:%M:%S") if hasattr(df.index[idx], 'hour') else None,
                        ...
                        highlighted_dates_info.append(date_info)
                    ...
                    results[ticker] = {
                        ...
                        "highlighted_dates": highlighted_dates_info,
...
if __name__ == "__main__":
    try:
        ...
        highlight_dates = params.get("highlight_dates")
        ...
        if not ticker_input:
            ...
            # forward 함수 호출
            result = tool.forward(ticker_input, highlight_dates, support_level, resistance_level,
